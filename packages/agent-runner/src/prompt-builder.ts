...
1. Write progress updates ONLY to .maestro/progress/${taskId}.json (the UI merges these automatically)
2. For progress during execution, write to .maestro/progress/${taskId}.json with:
   {"status":"in-progress","progress":"<current step>"}
3. When done, use the helper script: node .maestro/bin/task-done.cjs ${taskId} --commit <short-hash>
...
The summary should be written for a product manager — describe the user-facing outcome, not the code changes.`;
  ...
  if (/^\/codehealth/.test(command)) {
    return `Execute the codehealth skill defined in .claude/skills/codehealth/SKILL.md. Scan the entire codebase, score all 11 dimensions, and write the results to .maestro/quality/latest.json.

This is a headless execution — do not wait for user input at any point.`;
  }

  if (/^\/autofix/.test(command)) {
    return `Execute the autofix skill defined in .claude/skills/autofix/SKILL.md. Read the codehealth backlog from .maestro/quality/backlog.json, pick the highest-priority fixable issues, and fix them.

Write progress updates to .maestro/progress/autofix.json as you work.

This is a headless execution — do not wait for user input at any point.`;
  }

  if (/^\/orchestrator\s+arrange/.test(command)) {
    return `Read .maestro/state.json and analyze all pending tasks. Organize them into a logical dependency graph — figure out which tasks depend on others and set the dependsOn fields. Group related tasks under epics.

CRITICAL — DO NOT WRITE TO .maestro/state.json DIRECTLY. Other agents and the UI write to state.json concurrently. If you overwrite it, you will destroy other agents' work.

Instead, write ALL your changes to .maestro/progress/arrange.json. The UI merges this automatically. Use this exact JSON format:
